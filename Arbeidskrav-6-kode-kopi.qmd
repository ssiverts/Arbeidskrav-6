...
Ein para t-test og lineær regresjonsmodell vart gjennomført for å analysere skilnaden av skilnaden i muskelmasse frå pre til post mellom singelsett og 3sett.
...
leanmass_change_plot <-leanmass %>%
  ...
  labs(title = "Figur 1. Endring i muskelmasse for singelsett vs 3sett",
       x = "Tid (pre/post)",
       y = "Muskelmasse(g)")
...
Endring i muskelmasse for singlesett vs 3sett er visualisert i Figur 1. Muskelmasse auka i gjennomsnitt med `r avgleanmass_change_single`g (`r percentincrease_single`%) for singelsett og `r avgleanmass_change_multiple`g (`r percentincrease_multiple`%) for 3sett. Dette gav ein skilnad i auke på `r m1results` mellom treningsvolum, noko som tilsvara ein `r percentdifference`% større auke i muskelmasse for 3sett samanlikna med singelsett.
...
## Diskusjon
Denne studien fann ein auke i muskelmasse for både treningsvolum. Auka var signifikant større for 3sett enn for singelsett. Dette samsvarar med tidlegare originalstudiar [@Brigatto2019; @Aube2020; @SCHOENFELD2019] og metaanalyse  [@Schoenfeld2016] som har sett på effekten av ulike treningsvolum på muskelmasse.
Ein svakheit ved denne studien er at den berre ser på skilnaden mellom eitt og tre sett. Ettersom treningsøktene vart utført 2-3 gongar i veka, tilsvarar det 2-3 sett i veka for singelsett og 6-9 sett i veka for 3sett. Både desse forholda er relativt små volum, samanlikna med kva forsking seier er optimalt for hypertrofi  [@Brigatto2019; @BazValle2022], og denne studien sin inferens er difor kanskje noko avgrensa.
...
Studien fant at både treningsvolum auka muskelmasse, der 3sett hadde ein signifikant større auke enn singelsett. Grunna kunstig låge treningsvolum og at kvar deltakar var testperson for både treningsvolum samstundes, har studien avgrensa inferens og praktisk tyding.
